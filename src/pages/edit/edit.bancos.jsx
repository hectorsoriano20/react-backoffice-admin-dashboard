...
const EditBancos = ({inputs, title}) => {
    ...
    useEffect(() => {
        const fetchPinta = async () => {
            try {
                ...
                console.log(err);
                alert('Hubo un error al cargar el banco. Por favor, inténtalo de nuevo.');
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.put(`https://nodejs-sequelize-restapi-mssql-production.up.railway.app/api/v1/BancoSangre/PUT/${id}`, formData);
            console.log(response.data);
            alert('Banco editado correctamente'); 
            navigate(`/bancos/${id}`);
        } catch (err) {
            console.log(err);
            setIsLoading(false);
            alert('Hubo un error al editar el banco. Por favor, inténtalo de nuevo.');
        }
    }
